...
bool CBlock::AcceptBlock()
{
    ...
    //    return DoS(100, error("AcceptBlock() : reject too new nVersion = %d", nVersion)); // todo: reenable when switching protocol, don´t forget to up the block version
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime, nHeight))
        return DoS(50, error("AcceptBlock() : coinbase timestamp is too early  nTimeBlock=%d nTimeTx=%u", GetBlockTime(), FutureDrift((int64_t)vtx[0].nTime, nHeight)));
    ...
    // todo: find out why it doesn´t work here
